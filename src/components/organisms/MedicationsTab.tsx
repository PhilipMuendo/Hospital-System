...
export function MedicationsTab({ patientId }: { patientId: string | null }) {
  ...
  if (prescriptionsQuery.isLoading) {
    return <Skeleton className="h-40 rounded-[var(--radius-sm)]" />
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      {prescriptions.map((p) => (
        <div
          key={p.id}
          className="rounded-[var(--radius-sm)] border border-white/6 bg-surface-800/50 px-4 py-3.5"
        >
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate text-[13.5px] font-medium text-mist-50">{p.prescriber.name}</p>
              <p className="text-[11.5px] text-mist-600">{formatDate(p.createdAt)}</p>
            </div>
            <Badge status={statusTone[p.status]}>{statusLabel[p.status]}</Badge>
          </div>

          {p.notes && <p className="mt-2 text-[13px] text-mist-300">{p.notes}</p>}
          ...
          {p.allergyOverrideReason && (
            <p
              className="mt-2 rounded-[var(--radius-2xs)] border px-2.5 py-1.5 text-[12px]"
              style={{
                borderColor: 'color-mix(in srgb, var(--color-status-critical) 30%, transparent)',
                background: 'color-mix(in srgb, var(--color-status-critical) 10%, transparent)',
...
function MedicationRow({
  ...
  return (
    <div className="rounded-[var(--radius-2xs)] border border-white/6 bg-surface-900/40 px-3 py-2.5">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div className="min-w-0">
          <p className="truncate text-[13px] text-mist-100">
            {item.drug.genericName} {item.drug.strength}
            {item.drug.brandName && <span className="text-mist-500"> ({item.drug.brandName})</span>}
            {item.drug.controlled && (
              <span
                className="ml-1.5 rounded-full border px-1.5 py-0.5 text-[9.5px] font-semibold uppercase tracking-wide"
                style={{
          ...
          {item.instructions && (
            <p className="mt-1 text-[11.5px] text-mist-400">{item.instructions}</p>
          )}
        </div>

        <div className="shrink-0 text-right">
          <p className="font-mono text-[12.5px] tabular text-mist-200">
            {item.quantityDispensed}/{item.quantityPrescribed} {item.drug.unit}
          </p>
          <p className="font-mono text-[11px] tabular text-mist-600">{formatKES(charge)}</p>
        </div>
      </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        ...
        {item.dispenseEvents.length > 0 && (
          <button
            ...
            className="text-[11.5px] text-mist-500 hover:text-mist-300"
          >
            {expanded ? 'Hide' : `${item.dispenseEvents.length} dispense record${item.dispenseEvents.length > 1 ? 's' : ''}`}
          </button>
        )}

        {error && <span className="text-[11.5px] text-status-critical">{error}</span>}
      </div>
          ...
          >
            <div className="mt-2 flex flex-col gap-1.5 border-t border-white/6 pt-2">
              {item.dispenseEvents.map((event) => (
                <div key={event.id} className="text-[11.5px] text-mist-500">
                  <span className="text-mist-300">{event.quantity} {item.drug.unit}</span>
                  {' · '}
                  ...
                  <span className="ml-1 font-mono text-mist-600">
