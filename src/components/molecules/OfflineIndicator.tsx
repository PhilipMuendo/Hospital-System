...
export function OfflineIndicator() {
  ...
  return (
    <div className="no-print fixed bottom-4 right-4 z-50">
      {open && queue.length > 0 && (
        <div className="mb-2 max-h-72 w-80 overflow-y-auto rounded-sm border border-line bg-sunken/95 p-3 shadow-xl backdrop-blur">
          <p className="mb-2 text-2xs font-medium uppercase tracking-wide text-ink-600">
            Waiting to sync
          </p>
          {queue.map((q) => (
            <div key={q.id} className="mb-1.5 rounded-xs bg-header px-2.5 py-2">
              <p className="text-xs text-ink-800">{q.label}</p>
              ...
              {q.lastError && <p className="mt-0.5 text-2xs text-critical">{q.lastError}</p>}
            </div>
          ))}
          {stuck.length > 0 && (
            <p className="mt-2 text-2xs text-critical">
      ...
      <button
        ...
        style={{
          borderColor: online ? 'rgb(255 255 255 / 0.12)' : 'var(--color-warning)',
          background: online ? 'rgb(12 14 17 / 0.9)' : 'color-mix(in srgb, var(--color-warning) 18%, #0c0e11)',
        }}
      >
        <span
        ...
        {queue.length > 0 && (
          <span className="rounded-full bg-white/12 px-2 py-0.5 font-mono text-2xs tabular text-ink-900">
            {queue.length}
          </span>
        )}
        {online && queue.length > 0 && (
          <span
            ...
            className="ml-1 rounded-full bg-primary-600 px-2.5 py-0.5 text-2xs font-semibold text-white"
          >
